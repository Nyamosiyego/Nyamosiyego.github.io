...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-900 uppercase font-bold">Subjects</h2>
            </div>
            <div class="flex flex-wrap items-center">
                <a href="{{ route('subject.create') }}" class="bg-blue-900 text-white text-sm uppercase py-2 px-4 flex items-center rounded">
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            ...
            @foreach ($subjects as $subject)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-900 tracking-tight">{{ $subject->name }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-900 tracking-tight">{{ $subject->subject_code }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-900 tracking-tight">{{ $subject->teacher->user->name }}</div>
                    <div class="w-3/12 px-4 py-3 text-sm text-gray-900 tracking-tight">{{ $subject->description }}</div>
                    <div class="w-2/12 flex items-center justify-end px-3">
                        ...
                        <form action="{{ route('subject.destroy',$subject->id) }}" method="POST" class="inline-flex ml-1">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-red-900 block p-1 border border-red-600 rounded-sm">
