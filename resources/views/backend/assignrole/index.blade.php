...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-black uppercase font-bold">Assign Role</h2>
            </div>
            <div class="flex flex-wrap items-center">
                <a href="{{ route('assignrole.create') }}" class="bg-blue-900 text-white text-sm uppercase py-2 px-4 flex items-center rounded">
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            ...
            @foreach ($users as $user)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-900 tracking-tight">{{ $user->name }}</div>
                    <div class="w-5/12 px-4 py-3 text-sm font-semibold text-gray-900 tracking-tight">{{ $user->email }}</div>
                    <div class="w-2/12 px-4 py-3 flex flex-wrap">
                        @foreach ($user->roles as $role)
                            <span class="bg-gray-400 text-xs font-semibold text-gray-900 tracking-tight px-3 py-1 border rounded-full">{{ $role->name }}</span>
                        @endforeach
                    </div>
