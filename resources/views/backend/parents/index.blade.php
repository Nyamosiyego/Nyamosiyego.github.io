...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-black uppercase font-bold">Parents</h2>
            </div>
            <div class="flex flex-wrap items-center">
                <a href="{{ route('parents.create') }}" class="bg-blue-900 text-white text-sm uppercase py-2 px-4 flex items-center rounded">
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            ...
            @foreach ($parents as $parent)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-900 tracking-tight">{{ $parent->user->name }}</div>
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-900 tracking-tight">{{ $parent->user->email }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-900 tracking-tight">
                        @foreach ($parent->children as $children)
                            <span class="bg-gray-300 text-xs font-normal px-2 py-px border rounded-full inline-flex my-px">
                                {{ $children->user->name }}
                            </span>
                        @endforeach
                    </div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-900 tracking-tight">{{ $parent->phone }}</div>
                    <div class="w-2/12 flex items-center justify-end px-3">
                        ...
                        <a href="{{ route('parents.destroy', $parent->id) }}" data-url="{{ route('parents.destroy', $parent->id) }}" class="deletebtn ml-1 bg-red-900 block p-1 border border-red-600 rounded-sm">
