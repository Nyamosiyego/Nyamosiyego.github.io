...
@section('content')
    <div class="create">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-black uppercase font-bold">Attendance for class :- {{ $class->class_name }}</h2>
            </div>
                ...
                <form action="{{ route('teacher.attendance.store') }}" method="POST">
                    @foreach ($class->students as $student)
                        <div class="flex items-center justify-between border border-gray-200">
                            @csrf
                            <div class="w-4/12 text-sm text-left text-black py-2 px-4 font-semibold">{{ $student->user->name }}</div>
                            <div class="w-3/12 text-sm text-left text-black py-2 px-4 font-semibold">{{ $student->roll_number }}</div>
                            <div class="w-5/12 text-sm text-right py-2 px-4 flex items-center justify-end">
                                <label class="block text-blue-900 font-semibold sm:border-r sm:pr-4">
                                    <input name="attendences[{{ $student->id }}]" class="leading-tight" type="radio" value="present">
                                    <span class="text-sm">Present</span>
                                </label>
                                <label class="ml-4 block text-blue-900 font-semibold">
